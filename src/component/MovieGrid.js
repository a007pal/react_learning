...
export default function MovieGrid({movies, watchList, toggleWatchList}) {
    ...
    return (
        <div>
            <input className="search-input" type="text" placeholder="Search Movie..." value={searchTerm}
            ...
            <div className="movies-grid">
                {
                    filteredMovies.map(movie => (
                        <MovieCard movie={movie} toggleWatchList={toggleWatchList}
                                   isWatchListed={watchList.includes(movie.id)} key={movie.id}/>
                    ))
                }
            </div>
        </div>
